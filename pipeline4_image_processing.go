...
func walkFiles(done <-chan struct{}, root string) (<-chan string, <-chan error) {
	...
	go func() {
		...
		errc <- filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
			...
			// following lines are the function literal passed as second argument of Walk function
			// filter out error
			if err != nil {
				return err
			}
...
func processImage(done <-chan struct{}, paths <-chan string) <-chan *result {
	...
	const numbThumbnailer = 5
	var wg sync.WaitGroup
	wg.Add(numbThumbnailer)
	for i := 0; i < numbThumbnailer; i++ {
		go func() {
			thumbnailer()
			wg.Done()
		}()
